...
- **Last checkpoint:** 7 — sqlc setup
- **Next task:** checkpoint 8 — `internal/auth` (argon2id) + seed command
...
### ⬜ Checkpoint 8 — `internal/auth` + seed

**`internal/auth`** — argon2id `HashPassword` / `CheckPasswordHash`. Also used for
...
and **gated on `PLATFORM=dev`** — never applied to production (§7.5); the first real
production admin is provisioned out-of-band. §7.3 recommends a Go command
(`cmd/seed/main.go`) using the app's own `CreateUser`, so the seed can't drift from the
schema — add a column and it either uses it or fails to compile.

**Nothing else is seeded** (§7.4) — no CCs, files, audit rows, signatures or tokens.
