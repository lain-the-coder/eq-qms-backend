...
- **Last checkpoint:** 8 — auth + seed · **all pre-API plumbing complete**
...
once, with nothing hand-wired. All pre-API plumbing is now complete.
...
`internal/auth` (`MakeJWT`) written for the first time.

**Then, in API Endpoint Plan order:** `middlewareAuth` → `GET /api/me` → refresh/revoke
(sliding window) → user management (incl. the `FOR UPDATE` transaction) → CC
